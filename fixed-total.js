        ...
        <div class="sidebar scroll">
            <details open>
                ...
                <fieldset>
                    <legend>
                        Username
                        <a href="{{WIKI}}/username" title="Help" target="_blank" rel="noopener noreferrer">help</a>
                    </legend>
                    <div class="multiInput">
                        <div class="input-wrapper">
                            <input class="input" id="username" type="text" priv>
                        </div>
                        <button id="username">me</button>
                    </div>
                </fieldset>
                <hr>
                <fieldset>
...
`);
	...
	class UndiscordCore {
	  ...
	  async filterResponse() {
        ...
        messagesToDelete = messagesToDelete.filter(msg => msg.author.username == this.options.username);
...
`;
    ...
    function getUsername() {
        const LS = document.body.appendChild(document.createElement('iframe')).contentWindow.localStorage;
        const user_state = JSON.parse(LS.MultiAccountStore)._state.users;
        for(let i in user_state){
            if(user_state[i].id==parseInt(JSON.parse(LS.user_id_cache))){
                return user_state[i].username;
            }
        }
	...
	function initUI() {
      ...
      $('button#username').onclick = () => $('input#username').value = getUsername();
	  ...
	  fileSelection.onchange = async () => {
        ...
        $('input#username').value = getUsername();
	    try {
	      const file = files[0];
